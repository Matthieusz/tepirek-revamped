...
effectIt.layer(squadBuilderIntegrationTestLayer, { excludeTestServices: true })(
  "Drizzle account import and refetch integration",
  (it) => {
    it.effect("lists only Margonem accounts owned by the actor", () =>
      Effect.gen(function* testEffect() {
        ...
        yield* Effect.promise(() =>
          testDb.insert(margonemAccount).values({
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* testEffect() {
          ...
          const [removedCharacter] = yield* Effect.promise(() =>
            testDb
          ...
          yield* Effect.promise(() =>
            testDb.insert(margonemAccountRefetchPreviewCharacter).values([
              {
                avatarUrl: null,
                characterId: 1_296_632,
                level: 301,
          ...
          const storedCharacters = yield* Effect.promise(() =>
            testDb
              .select({
    ...
    it.effect("confirms pending account imports through the Effect store", () =>
      Effect.gen(function* testEffect() {
        ...
        yield* Effect.promise(() =>
          testDb.insert(margonemAccountImportPreviewCharacter).values({
            avatarUrl: null,
        ...
        const [storedAccount] = yield* Effect.promise(() =>
          testDb
