...
describe("Effect HttpApi routes", () => {
  ...
  effectIt.effect(
    "serves bet routes through the final HttpApi handler seam",
    () =>
      withAppHttpApi(async (appHttpApi) => {
        ...
        const listResponse = await requestHttpApi(
          appHttpApi,
          "/bet",
          authenticatedGet(cookie)
        );
  ...
  effectIt.effect(
    "serves ranking routes through the final HttpApi handler seam",
    () =>
      withAppHttpApi(async (appHttpApi) => {
  ...
  effectIt.effect(
    "serves vault routes through the final HttpApi handler seam",
    () =>
      withAppHttpApi(async (appHttpApi) => {
        ...
        const vaultResponse = await requestHttpApi(
          appHttpApi,
          "/vault",
          jsonPost({}, cookie)
        );
